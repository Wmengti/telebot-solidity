...
contract TokenCheckerTax {
  ...
  constructor(address _uniswapV2Router) payable {
  ...
  function buy(address tokenAddress, uint256 amountEthIn) internal returns (uint256, uint256) {
    ...
    uniswapRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amountEthIn}(
      0,
      eth2TokenPath,
      senderAddress,
  ...
  function sell(address tokenAddress, uint256 amountTokenIn) private returns (uint256, uint256) {
    ...
    require(token.approve(uniswapV2RouterAddress, amountTokenIn), "Approve Failed.");
